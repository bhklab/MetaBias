...
nintersects = 50; sets = NULL; keep.order = F; 
...
shade.color = "grey"; text.scale = 1.2; text.face = "bold";
point.size = 1.8; 
line.size = 0.75; 
att.pos = "top";
...
upset.fun <- function(data, plot.name){
  
  mainbar.y.label = plot.name  
  ...
  for(i in 1:length(sets)) {
    ...
    if(length(j) > 0) Matrix_layout$color[j] <- col.id[i]
  }
  ...
  Sizes <- UpSetR:::Make_size_plot(Set_sizes, sets.bar.color, mb.ratio, 
                                   sets.x.label, scale.sets, text.scale, 
                                   set_size.angles,set_size.show, set_size.scale_max,
                                   set_size.numbers_size)
  
  p<- structure(class = "upset",
            .Data=list(
              Main_bar = Main_bar,
              Matrix = Matrix,
              Sizes = Sizes,
